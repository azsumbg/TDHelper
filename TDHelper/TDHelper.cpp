...
void dll::ORCS::Move(BAG<ASSETS>& obstacles, float gear)
{
	...
	else
	{
		if (ver_dir)
		{
			if (move_ey < move_sy)   // MOVING UP
			{
				switch (move_flag)
				{
				case up_flag:
					if (center.x < scr_width / 2.0f)start.x += my_speed;
...
states dll::OrcAI(ORCS& myself, BAG<FPOINT>& buildings, BAG<ORCS>& orcs)
{
	...
	if (current == states::attack || current == states::heal || myself.bumped)return current;
